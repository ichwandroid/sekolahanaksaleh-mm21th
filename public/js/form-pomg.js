...
window.initForm = function () {
    console.log('Initializing Form POMG Script...');
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = {
                    ...
                    phone: phone,
                    email: email,
                    type: 'KB_TK_ANAK_SALEH',
                    source: 'form-pomg',
                    created_at: new Date().toISOString()
                };

                // Using 'registrations' collection. 
                // Note: The structure is different from KS/Guru, so ensure dashboard can handle it or use a separate collection if needed.
                // Assuming sticking to 'registrations' is fine for now, or maybe 'registrations_pomg' could be cleaner. 
                // But for simplicity and if dashboards look at 'registrations', I'll keep it there but with distinct fields.
                // Wait, if the dashboard expects 'child_name'/'parent_name', this might break listing. 
                // To be safe, let's map participant_name to child_name as well just in case legacy views need it.
                data.child_name = participantName;
                data.parent_name = '-';
                data.school_name = 'KB TK Anak Saleh';
                ...
                setTimeout(async () => {
                    ...
                    try {
                        ...
                        link.href = ticketUrl;
                        link.download = `E-Ticket_KBTK_${data.phone}.jpg`;
                        link.click();
...
// Helper: Generate Ticket Image for KB TK
async function generateTicket(data, proofImgUrl) {
    ...
    ctx.textAlign = 'left';
    ctx.fillText('E-TICKET PENDAFTARAN', 160, 75);
    ctx.font = '24px sans-serif';
    ctx.fillText('KB TK ANAK SALEH - MILAD 21', 160, 115);

    // QR Code
    try {
        const qrData = JSON.stringify({ id: data.id, checkin: true, type: 'KB_TK' });
    ...
    drawField('Nama Peserta', data.participant_name);

    y += 20;

    drawField('No WA', data.phone);
    ...
    ctx.fillText('Tiket ini sebagai bukti pendaftaran KB TK.', canvas.width / 2, footerY - 10);
    ctx.fillText('Simpan dan tunjukkan saat acara.', canvas.width / 2, footerY + 10);
...
function validateForm(formData) {
    ...
    if (!participantName) {
        throw new Error("Mohon lengkapi Nama Peserta.");
    }
